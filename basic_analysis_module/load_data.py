...
def load_TMDS_file_multi(file_path_list):
    """
    加载tdms文件（多个）
    ...
    for file_path in file_path_list:
        samp_v.extend(load_TMDS_file(file_path))
    return np.array(samp_v)


def get_current(samp_v, device_id, key_para):
    if device_id == 0:
        current = get_current_id_0(samp_v, key_para)
    elif device_id==1:
        current=get_current_id_1(samp_v,key_para)
    elif device_id==2:
        current=get_current_id_2(samp_v,key_para)
    elif device_id==3:
        current=get_current_id_3(samp_v,key_para)
    return current


def get_current_id_0(samp_v, key_para):
    currentPre = get_currentPre_id_0(samp_v, key_para)
    background = cal_background(currentPre)
    current = remove_bg(currentPre, background)
    return current

def get_current_id_1(samp_v, key_para):
    currentPre = get_currentPre_id_1(samp_v, key_para)
    background = cal_background(currentPre)
    current = remove_bg(currentPre, background)
    return current

def get_current_id_2(samp_v, key_para):
    currentPre = get_currentPre_id_2(samp_v, key_para)
    background = cal_background(currentPre)
    current = remove_bg(currentPre, background)
    return current

def get_current_id_3(samp_v, key_para):
    currentPre = get_currentPre_id_3(samp_v, key_para)
    background = cal_background(currentPre)
    current = remove_bg(currentPre, background)
    return current

def get_currentPre_id_0(samp_v, key_para):
    """
    STM41的电流计算
    :param samp_v:采样电压
    :param key_para: 拟合参数
...
def get_currentPre_id_1(samp_v, key_para):
    """
    STM40的电流计算
    :param samp_v:采样电压
    :param key_para: 拟合参数
...
def get_currentPre_id_2(samp_v, key_para):
    """
    MCBJ41的电流计算
    :param samp_v:采样电压
    :param key_para: 拟合参数
...
def get_currentPre_id_3(samp_v, key_para):
    """
    STMNEW的电流计算
    :param samp_v:采样电压
    :param key_para: 拟合参数
...
def get_logG(file_path, key_para):
    """
    计算电导值
    ...
    :return: 电导
    """
    samp_v = load_TMDS_file(file_path)
    device_id = key_para["DEVICE_ID"]
    current = get_current(samp_v, device_id, key_para)
    bias_V = key_para["le_BiasV"]
    log_G = np.log10(abs(current * 12886.6 / bias_V))
    return log_G
...
def cut_trace(log_G, key_para):
    SELECT_OPTION = key_para["SELECT_OPTION"]
    if key_para["PROCESS"] == 0:
        if not SELECT_OPTION:
            cut_open_trace(log_G, key_para)
    else:
        cut_close_trace(log_G, key_para)


def cut_open_trace(log_G, key_para):
    # TODO 这里需要记得把lowcut去掉了

    HIGH_CUT = key_para["le_High_Cut"]
    LOW_CUT = key_para["le_Low_Cut"]
    ...
    STEP = get_step_from_sampling(SAMPLING_RATE)
    ...
    start, zero, end, len_high, len_low = np.array(list(start.values()))[:TRUE_LENGTH], np.array(list(zero.values()))[
...
def cut_open_trace_with_select(log_G, key_para):
    HIGH_CUT = key_para["le_High_Cut"]
    LOW_CUT = key_para["le_Low_Cut"]
    HIGH_LENGTH = key_para["le_High_Length"]
    LOW_LENGTH = key_para["le_Low_Length"]
    ...
    while (index < ENDINDEX):
        try:
            if (np.mean(log_G[index - IS_DECLINE_STEP:index]) > np.mean(log_G[index:index + IS_DECLINE_STEP])):
                ...
                if n in start.keys() and n in zero.keys() and n in len_high.keys() and n in start1.keys() and n in end1.keys() and n in start2.keys() and n in end2.keys() and n in len_low.keys() and len_low.get(
                        n) > len_high.get(n) >= zero.get(n) > start.get(n):
                    n += 1
                    index += JUMP_GAP
                    continue
            ...
            print(f"切分单条时发生异常：{e}")
            break
    TRUE_LENGTH = min(len(start), len(zero), len(end), len(len_low), len(len_high), len(start1), len(end1), len(start2),
                      len(end2))-1
...
def cut_close_trace(log_G, key_para):
    pass


def get_step_from_sampling(SAMPLING_RATE):
    """
    根据采样频率自动计算步长
    :param SAMPLING_RATE:
    :return: 步长（int）
