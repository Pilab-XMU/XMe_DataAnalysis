...
from ui_QWSingleTraceAnalysisModule import Ui_QWSingleEgainAnalysisModule
...
class QmySingleTraceAnalysisModule(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.ui = Ui_QWSingleEgainAnalysisModule()
    ...
    def init_widget_para(self):
        ...
        self.lastOpenPath = GeneralUtils.getDesktopPath()
        self.file_path = ""
        self.saveFolderName = ""
        self.saveFolderPath = ""
    ...
    def on_actOpenFiles_triggered(self):
        ...
        try:
            ...
            while not loadStatue:
                filePath, _ = QFileDialog.getOpenFileName(self, dlgTitle, self.lastOpenPath, filt)
    ...
    def on_btn_Next_Trace_clicked(self):
        ...
        if self.CURRENT_INDEX == self.TRACE_NUM - 1:
            self.ui.btn_Next_Trace.setEnabled(False)
        if self.CURRENT_INDEX == self.TRACE_NUM:
            self.CURRENT_INDEX = self.TRACE_NUM -1
            return
    ...
    def init_dataset(self, file_path):
        ...
        self.x_p, self.y_p, self.x_m, self.y_m = dataset['xp'], dataset['yp'], dataset['xm'], dataset['ym']
        self.TRACE_NUM = np.min([len(self.x_p), len(self.x_m)])
    ...
    def draw_fig(self):
        ...
        x_p, y_p = self.x_p[i], self.y_p[i]
        x_m, y_m = self.x_m[i], self.y_m[i]
        ax = self.fig.add_subplot()
        if self.view == 0:
            ax.semilogy(x_p, y_p, c='r',label='forward')
            ax.semilogy(x_m, y_m, c='b',label='reverse')
        elif self.view == 1:
            ax.semilogy(x_p, y_p, c='r',label='forward')
        else:
            ax.semilogy(x_m, y_m, c='b',label='reverse')
        # ax.plot(x_p, logy_p, c='r',label='forward')
        # ax.plot(x_m, logy_m, c='b',label='reverse')
    ...
    def save_select_data(self):
        ...
        if "npz" == self.save_format:
            ...
            x_for, y_for = self.x_p[select_index], self.y_p[select_index]
            x_rev, y_rev = self.x_m[select_index], self.y_m[select_index]
            np.savez(saveFilePath, xp= x_for, yp=y_for, xm=x_rev, ym=y_rev)
        else:
            for i, idx in enumerate(select_index):
                x_for, y_for = self.x_p[idx], self.y_p[idx]
                x_rev, y_rev = self.x_m[idx], self.y_m[idx]
                arr = np.column_stack([x_for, y_for, x_rev, y_rev])
                np.savetxt(os.path.join(saveFolderPath, f'{i}.txt'), arr, delimiter='\t', fmt='%.8f')
                # with open(os.path.join(saveFolderPath, f'{i}.txt'), 'w') as f:
                #     line1 = '\t'.join(['%.8f'%elem for elem in x_for])
                #     f.write(line1 + '\n')
                #     line2 = '\t'.join(['%.8f'%elem for elem in y_for])
                #     f.write(line2 + '\n')
                #     line3 = '\t'.join(['%.8f'%elem for elem in x_rev])
                #     f.write(line3 + '\n')
                #     line4 = '\t'.join(['%.8f'%elem for elem in y_rev])
                #     f.write(line4 + '\n')
